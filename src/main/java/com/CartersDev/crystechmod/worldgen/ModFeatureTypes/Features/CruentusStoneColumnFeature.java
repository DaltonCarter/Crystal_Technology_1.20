...
public class CruentusStoneColumnFeature extends Feature<ColumnFeatureConfiguration> {
    ...
    public CruentusStoneColumnFeature(Codec<ColumnFeatureConfiguration> pCodec) {
        super(pCodec);
    }
            ...
            if (blockpos1 != null) {
                ...
                for(BlockPos.MutableBlockPos blockpos$mutableblockpos = blockpos1.mutable(); j >= 0; --j) {
                    if (isAirOrLavaOcean(pLevel, pSeaLevel, blockpos$mutableblockpos)) {
                        this.setBlock(pLevel, blockpos$mutableblockpos, ModBlocks.CRUENTUS_STONE.get().defaultBlockState());
                        blockpos$mutableblockpos.move(Direction.UP);
                        flag = true;
                    } else {
                        if (!pLevel.getBlockState(blockpos$mutableblockpos).is(ModBlocks.CRUENTUS_STONE.get())) {
                            break;
                        }
