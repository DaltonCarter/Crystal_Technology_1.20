...
public class ViniferaCrystalColumnFeature extends Feature<ColumnFeatureConfiguration> {
    ...
    public ViniferaCrystalColumnFeature(Codec<ColumnFeatureConfiguration> pCodec) {
        super(pCodec);
    }
            ...
            if (blockpos1 != null) {
                ...
                for(BlockPos.MutableBlockPos blockpos$mutableblockpos = blockpos1.mutable(); j >= 0; --j) {
                    if (isAirOrLavaOcean(pLevel, pSeaLevel, blockpos$mutableblockpos)) {
                        this.setBlock(pLevel, blockpos$mutableblockpos, ModBlocks.VINIFERA_CRYSTAL.get().defaultBlockState());
                        blockpos$mutableblockpos.move(Direction.UP);
                        flag = true;
                    } else {
                        if (!pLevel.getBlockState(blockpos$mutableblockpos).is(ModBlocks.VINIFERA_CRYSTAL.get())) {
                            break;
                        }
