...
public class TiberiumGrinderBlockEntityRenderer implements BlockEntityRenderer<TiberiumGrinderBlockEntity> {

    public TiberiumGrinderBlockEntityRenderer(BlockEntityRendererProvider.Context context) {
    ...
    @Override
    public void render(TiberiumGrinderBlockEntity pBlockEntity, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, int pPackedOverlay) {
        ItemRenderer itemRenderer = Minecraft.getInstance().getItemRenderer();
        ItemStack itemStack = pBlockEntity.getRenderStack();

        pPoseStack.pushPose();
        pPoseStack.translate(0.5f, 0.84f, 0.5f);
