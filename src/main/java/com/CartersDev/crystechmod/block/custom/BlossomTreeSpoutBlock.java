...
public class BlossomTreeSpoutBlock extends RotatedPillarBlock {
    ...
    public void animateTick(BlockState state, Level world, BlockPos pos, RandomSource rand) {
        ...
        world.addParticle(ModParticles.CONTAMINATION_PARTICLES.get(), pos.getX() + rand.nextDouble(),
                pos.getY() + 1.5D, pos.getZ() + rand.nextDouble(),
                0d, 0.05d, 0d);

        if(chance < rand.nextFloat()) {
            world.playSound(null, x, y, z, SoundEvents.CREEPER_PRIMED, SoundSource.BLOCKS, 3.0f, 1.0f);
    ...
    public void infectionTick (BlockPos pos, ServerLevel worldIn) {
        Random random = new Random();

        worldIn.addParticle(ModParticles.INFECTION_PARTICLES.get(), pos.getX() + random.nextDouble(),
                pos.getY() + 0.7D, pos.getZ() + random.nextDouble(),
                0d, 0.05d, 0d);
    }
