...
public class ImprovedLaserBeamEntity extends Projectile {

    private float baseDamage = 2.0F;

    private static final EntityDataAccessor<Boolean> HIT =
            SynchedEntityData.defineId(ImprovedLaserBeamEntity.class, EntityDataSerializers.BOOLEAN);

    private static boolean igniteTarget = false;
    private static boolean detonate = false;

    private static final EntityDataAccessor<Byte> PIERCE_LEVEL = SynchedEntityData.defineId(ImprovedLaserBeamEntity.class, EntityDataSerializers.BYTE);
    ...
    public ImprovedLaserBeamEntity(EntityType<? extends Projectile> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
    }

    public ImprovedLaserBeamEntity(Level pLevel, Player player) {
        super(ModEntities.IMPROVED_LASER_BEAM.get(), pLevel);
        setOwner(player);
...
@Override
    ...
    for(int x = 0; x < 18; ++x) {
        for(int y = 0; y < 18; ++y) {
            this.level().addParticle(ParticleTypes.DRAGON_BREATH, partPos.getX(), partPos.getY(), partPos.getZ(),
    ...
    protected void onHitEntity(EntityHitResult pResult) {
        ...
        if (hurt) {
            if(hitEntity instanceof LivingEntity livingHitEntity) {
                livingHitEntity.addEffect(new MobEffectInstance(MobEffects.WEAKNESS, 100, 2), owner);
            }
        }
        ...
        for(int x = 0; x < 18; ++x) {
            for(int y = 0; y < 18; ++y) {
                this.level().addParticle(ParticleTypes.DRAGON_BREATH, hitEntity.getX(), hitEntity.getY(), hitEntity.getZ(),
                        Math.cos(x*20) * 0.15d, Math.cos(y*20) * 0.15d, Math.sin(x*20) * 0.15d);
            }
        }
