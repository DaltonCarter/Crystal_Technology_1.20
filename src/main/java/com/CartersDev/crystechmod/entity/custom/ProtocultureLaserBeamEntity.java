...
public class ProtocultureLaserBeamEntity extends Projectile {

    private float baseDamage = 2.0F;

    private static final EntityDataAccessor<Boolean> HIT =
            SynchedEntityData.defineId(ProtocultureLaserBeamEntity.class, EntityDataSerializers.BOOLEAN);

    private static boolean igniteTarget = false;
    private static boolean detonate = false;

    private static final EntityDataAccessor<Byte> PIERCE_LEVEL = SynchedEntityData.defineId(ProtocultureLaserBeamEntity.class, EntityDataSerializers.BYTE);
    ...
    public ProtocultureLaserBeamEntity(EntityType<? extends Projectile> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
    }

    public ProtocultureLaserBeamEntity(Level pLevel, Player player) {
        super(ModEntities.PROTOCULTURE_LASER_BEAM.get(), pLevel);
        setOwner(player);
...
@Override
    ...
    for(int x = 0; x < 18; ++x) {
        for(int y = 0; y < 18; ++y) {
            this.level().addParticle(ModParticles.PROTOCULTURE_PARTICLES.get(), partPos.getX(), partPos.getY(), partPos.getZ(),
    ...
    protected void onHitEntity(EntityHitResult pResult) {
        ...
        if (hurt) {
            if(hitEntity instanceof LivingEntity livingHitEntity) {
                livingHitEntity.addEffect(new MobEffectInstance(MobEffects.CONFUSION, 100, 2), owner);
            }
        }
        ...
        for(int x = 0; x < 18; ++x) {
            for(int y = 0; y < 18; ++y) {
                this.level().addParticle(ModParticles.PROTOCULTURE_PARTICLES.get(), hitEntity.getX(), hitEntity.getY(), hitEntity.getZ(),
                        Math.cos(x*20) * 0.15d, Math.cos(y*20) * 0.15d, Math.sin(x*20) * 0.15d);
            }
        }
    ...
    public boolean setDetonate(boolean detonate) {
        return ProtocultureLaserBeamEntity.detonate = detonate;
    }
