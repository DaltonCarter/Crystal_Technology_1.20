...
public class AdvancedLaserBeamEntity extends Projectile {

    private float baseDamage = 2.0F;

    private static final EntityDataAccessor<Boolean> HIT =
            SynchedEntityData.defineId(AdvancedLaserBeamEntity.class, EntityDataSerializers.BOOLEAN);

    private static boolean igniteTarget = false;
    private static boolean detonate = false;

    private static final EntityDataAccessor<Byte> PIERCE_LEVEL = SynchedEntityData.defineId(AdvancedLaserBeamEntity.class, EntityDataSerializers.BYTE);
    ...
    public AdvancedLaserBeamEntity(EntityType<? extends Projectile> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
    }

    public AdvancedLaserBeamEntity(Level pLevel, Player player) {
        super(ModEntities.ADVANCED_LASER_BEAM.get(), pLevel);
        setOwner(player);
    ...
    protected void onHitEntity(EntityHitResult pResult) {
        ...
        if (hurt) {
            if(hitEntity instanceof LivingEntity livingHitEntity) {
                livingHitEntity.addEffect(new MobEffectInstance(MobEffects.WEAKNESS, 100, 1), owner);
            }
        }
