...
public class LaserBeamRenderer extends EntityRenderer<LaserBeamEntity> {
    ...
    protected LaserBeamModel model;

    public LaserBeamRenderer(EntityRendererProvider.Context pContext) {
        super(pContext);
        model = new LaserBeamModel(pContext.bakeLayer(ModModelLayers.LASER_BEAM_LAYER));
        this.shadowRadius = 0.5F;
    }

    public void render(LaserBeamEntity entity, float pEntityYaw, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        pPoseStack.pushPose();
        ...
        this.model.renderToBuffer(pPoseStack, vertexconsumer, pPackedLight, OverlayTexture.NO_OVERLAY, 0f, 1f, 1f, 1f);
        pPoseStack.popPose();
    ...
    @Override
    public ResourceLocation getTextureLocation(LaserBeamEntity pEntity) {
        return TEXTURE;
    }
}
