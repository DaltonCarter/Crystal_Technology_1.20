...
public class BeamCaster extends ProjectileWeaponItem  {
    ...
    public static final Predicate<ItemStack> FOCUS = (p_43017_) -> {
        return p_43017_.is(ModTags.Items.FOCUSES);
    };

    public BeamCaster(Item.Properties pProperties) {
        super(pProperties);
    }
    ...
    public void spawnEmptyFocus(LivingAttackEvent event) {
        ...
        int breakThreshold = ammo.getMaxDamage() - 1;


        if (!event.getEntity().getCommandSenderWorld().isClientSide() && event.getSource().getEntity() instanceof Player pPlayer) {
            if (ammo.getDamageValue() >= breakThreshold) {
                if (!pPlayer.getInventory().add(emptyFocus)) {
                    pPlayer.spawnAtLocation(emptyFocus, 0.1F);
