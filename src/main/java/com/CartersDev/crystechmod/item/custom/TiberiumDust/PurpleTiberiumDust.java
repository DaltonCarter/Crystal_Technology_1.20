package com.CartersDev.crystechmod.item.custom.TiberiumDust;
...
public class PurpleTiberiumDust extends ItemNameBlockItem {
    public PurpleTiberiumDust(Block pBlock, Properties pProperties) {
        ...
        if (!pLevel.isClientSide() && pHand == InteractionHand.MAIN_HAND) {
            if (itemstack.is(ModItems.PURPLE_TIBERIUM_DUST.get())) {
                int SoilChance = rand.nextInt(10);
                    if(pLevel.getBlockState(pPos).getBlock() == ModBlocks.ICHOR_SOIL.get() || pLevel.getBlockState(pPos).getBlock() == ModBlocks.INERT_ICHOR_SOIL.get()){
                        pLevel.playSound(null, pPos, SoundEvents.AMETHYST_BLOCK_RESONATE, SoundSource.BLOCKS, 1f, 1f);
                        pLevel.setBlockAndUpdate(pPos.above(), ModBlocks.PURPLE_TIBERIUM_CROP.get().defaultBlockState());
                        itemstack.shrink(1);
                        return InteractionResult.SUCCESS;
                    } else if (SoilChance >= 3) {
